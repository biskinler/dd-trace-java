...
public class ExecutorInstrumentationUtils {
  ...
  public static boolean shouldAttachStateToTask(final Object task, final Executor executor) {
    ...
    return (scope instanceof TraceScope
        ...
        && task != null
        // Disabled executors cannot accept wrapped tasks, non wrapped tasks (i.e. tasks with
        // injected fields) should still work fine.
        && !((task instanceof RunnableWrapper || task instanceof CallableWrapper)
            && ExecutorInstrumentationUtils.isDisabledForWrappedTasks(executor)));
  }
  ...
  public static boolean isDisabledForWrappedTasks(final Executor executor) {
    return EXECUTORS_DISABLED_FOR_WRAPPED_TASKS.containsKey(executor);
  }
}
